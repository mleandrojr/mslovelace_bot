...
use crate::repositories::{chats, messages, users};
...
pub async fn run(pool: MySqlPool, ctx: &Context) {
    ...
    let Ok(Some(db_user)) = users::find_by_user_id(&pool, ctx_user.data.id).await else { return };
...
async fn ensure_message(
    ...
    match messages::find_by_telegram_id(pool, ctx_message.data.message_id, chat_id).await {
        Ok(Some(m)) => { let _ = messages::update(pool, ctx_message).await; Some(m) }
        Ok(None) => messages::create(pool, user_id, chat_id, ctx_message).await.ok(),
...
async fn add_reply_to_message_id(pool: &MySqlPool, db_message: &crate::models::message::Message, reply_to_message_id: i32) {
    match messages::find_by_telegram_id(pool, reply_to_message_id, db_message.chat_id).await {
        Ok(Some(m)) => { let _ = messages::update_reply_to(pool, db_message, m.id).await; }
        Ok(None) => (),
...
async fn add_thread_id(pool: &MySqlPool, db_message: &crate::models::message::Message, thread_id: i32) {
    match messages::find_by_telegram_id(pool, thread_id, db_message.chat_id).await {
        Ok(Some(m)) => { let _ = messages::update_thread_id(pool, db_message, m.id).await; }
        Ok(None) => (),
